...
def appearance(intervals):
    pupil_on_lesson = {t for i in range(1, len(intervals['pupil']), 2) for t in range(
        intervals['pupil'][i-1], intervals['pupil'][i]) if t >= intervals['lesson'][0] and t < intervals['lesson'][1]}
    tutor_on_lesson = {t for i in range(1, len(intervals['tutor']), 2) for t in range(
        intervals['tutor'][i-1], intervals['tutor'][i]) if t >= intervals['lesson'][0] and t < intervals['lesson'][1]}

    return len(pupil_on_lesson.intersection(tutor_on_lesson))
